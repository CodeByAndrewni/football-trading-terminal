...
import { formatOddsDisplay, type KellyResult, calculateKellyWithRealOdds } from '../services/kellyCalculator';
...
interface MatchWithSignal extends AdvancedMatch {
  ...
  signalResult: SignalStrengthResult | null;
...
export function BattleRoomPage() {
  ...
  const processedMatches: MatchWithSignal[] = useMemo(() => {
    ...
    for (const match of rawMatches) {
      ...
      // 评分失败或无信号时，使用基于0强度的 Kelly 结果兜底
      const kellyResult: KellyResult =
        signalResult?.kellyResult ?? calculateKellyWithRealOdds(0, match);
        ...
        <div className="flex items-center gap-4 mb-4 px-3 py-2 bg-[#0d0d0d] rounded-lg text-sm">
          ...
          <span className="text-[#888]">
            ...
            <span className="text-[#00d4ff]">
              {signalResult?.components.timeMultiplier.toFixed(2) ?? '--'}x
            </span>
          </span>
        </div>
