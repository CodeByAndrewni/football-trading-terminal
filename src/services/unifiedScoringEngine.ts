...
export function formatSignalSummary(signal: UnifiedSignal): string {
  ...
  const actionLabel = ACTION_LABELS[signal.action];
...
export function validateSignal(
  ...
  const scoreHome = signal.reasons?.state?.score_home ?? 0;
  const scoreAway = signal.reasons?.state?.score_away ?? 0;
  const triggerScore = scoreHome + scoreAway;
  const finalScore = finalScoreHome + finalScoreAway;
  const goalsAfter = finalScore - triggerScore;

  // 找到触发后第一个进球
  const signalMinute = signal.minute ?? 0;
  const firstGoalAfter = goalMinutes.find(m => m > signalMinute);
...
export function aggregateBacktestResults(
  ...
  for (const v of moduleValidations) {
    const action = v.action ?? 'WATCH';
    byAction[action].count++;
    if (v.is_hit) byAction[action].hit_count++;
  }
  ...
  for (const v of moduleValidations) {
    const conf = v.trigger_confidence ?? 0;
    let range = '0-40';
    if (conf >= 80) range = '80-100';
    else if (conf >= 60) range = '60-80';
    else if (conf >= 40) range = '40-60';
