...
const SEED_LEAGUES: Record<string, string> = {
  ...
  '239': '哥伦比亚甲级联赛|哥甲', '240': '哥伦比亚乙级联赛|哥伦乙',
  '242': '厄瓜多尔甲级联赛|厄甲',
  '265': '巴拉圭甲级联赛|巴拉甲', '268': '乌拉圭甲级联赛|乌甲', '269': '乌拉圭乙级联赛|乌拉乙',
  '266': '秘鲁甲级联赛|秘甲',
...
interface PendingTeam { id: number; name: string; league: string; country: string }
...
export function collectUntranslatedTeams(
  ...
  for (const f of fixtures) {
    const country = (f.league as { country?: string }).country ?? '';
    for (const t of [f.teams.home, f.teams.away]) {
      if (seen.has(t.id) || teamCache[String(t.id)]) continue;
      seen.add(t.id);
      result.push({ id: t.id, name: t.name, league: f.league.name, country });
    }
  }
  return result;
}
...
- 必须结合「国家/地区」区分同名球队或不同国家的同名俱乐部
- 不要加括号注释、不要解释，每行只输出：ID=中文名

请翻译以下球队（每行格式：ID|英文名|所在联赛|国家或地区）：
`;
...
- 许多国家都有 Primera División、Liga、First Division 等同名联赛，必须根据第三列「国家/地区」区分，简称不得与其他国家混淆（例如 乌拉圭甲级联赛|乌拉甲、哥斯达黎加甲级联赛|哥斯甲、西班牙乙级联赛|西乙）。
- 每行输出：ID=全称|简称

请翻译以下联赛（每行格式：ID|英文名|国家或地区）：
`;
...
function parseTranslationResponse(text: string): Map<number, string> {
  ...
  for (const line of body.split('\n')) {
    ...
    if (!isNaN(id) && value && value.length > 0 && value.length < 64) {
      result.set(id, value);
    }
  }
  return result;
...
export async function translatePendingTeams(
  ...
  for (let i = 0; i < teams.length; i += BATCH_SIZE) {
    ...
    const lines = batch.map((t) => `${t.id}|${t.name}|${t.league}|${t.country}`).join('\n');
