...
export function MatchDetailPage() {
  ...
  const matchData = useMemo(() => {
    // PRODUCTION STRICT MODE: 只使用 API 数据
    if (!matchResult?.match || matchResult.dataSource !== 'api') {
      return null;
    }
    ...
    const statsList = [
      ...
      { label: 'xG', home: finalHomeXg, away: finalAwayXg },
  ...
  // ============================================
  // PRODUCTION STRICT MODE: NO LIVE DATA 显示
  // ============================================
  if (!isLoading && (!matchResult?.match || matchResult.dataSource !== 'api')) {
    return (
      <div className="min-h-screen bg-bg-deepest flex items-center justify-center">
        <div className="flex flex-col items-center gap-6 p-8 bg-bg-card rounded-2xl border border-border-default max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-text-primary">NO LIVE DATA</h2>
          <p className="text-text-secondary">
            无法从 API-Football 获取比赛数据。
          </p>
          <div className="text-xs text-text-muted bg-bg-elevated p-3 rounded-lg font-mono">
            PRODUCTION STRICT MODE<br/>
            所有数据必须来自 API-Football<br/>
            不支持模拟数据
          </div>
    ...
    return (
      <div className="min-h-screen bg-bg-deepest flex items-center justify-center">
        <div className="flex flex-col items-center gap-6 p-8 bg-bg-card rounded-2xl border border-border-default max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-text-primary">无法评分</h2>
          <p className="text-text-secondary">
            比赛数据不完整，无法进行评分计算。
          </p>
          {matchResult?.match?._unscoreable && (
            <div className="text-xs text-text-muted bg-bg-elevated p-3 rounded-lg font-mono">
              原因: {matchResult.match._noStatsReason || 'MISSING_STATISTICS_DATA'}
            </div>
          )}
          <Link
            to="/"
  ...
  return (
    <div className={`min-h-screen bg-bg-deepest ${showAlertAnimation ? 'animate-pulse' : ''}`}>
      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-6 py-6">
        {/* 比赛头部 - 比分和核心评分 */}
        <MatchHeaderSection match={matchData} scoreResult={scoreResult} />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-6">
          ...
          <div className="xl:col-span-8 space-y-6">
            {/* 评分面板 */}
            <ScoreDashboard match={matchData} scoreResult={scoreResult} />
          ...
          <div className="xl:col-span-4 space-y-6">
            ...
            {/* 雷达图 */}
            <FactorRadarChart factors={scoreResult.factors} />

            {/* 因子详情 */}
            <FactorBreakdownPanel factors={scoreResult.factors} />
...
// 比赛头部区域
// ============================================
function MatchHeaderSection({ match, scoreResult }: { match: AdvancedMatch; scoreResult: ScoreResult }) {
  const getScoreColor = () => {
    if (scoreResult.totalScore >= 80) return 'text-accent-danger';
    if (scoreResult.totalScore >= 60) return 'text-accent-warning';
    if (scoreResult.totalScore >= 40) return 'text-accent-success';
    return 'text-text-muted';
  };

  const getScoreBg = () => {
    if (scoreResult.totalScore >= 80) return 'from-accent-danger/20 to-transparent';
    if (scoreResult.totalScore >= 60) return 'from-accent-warning/20 to-transparent';
    return 'from-accent-primary/10 to-transparent';
  };

  return (
    <div className={`card relative overflow-hidden ${scoreResult.totalScore >= 80 ? 'ring-2 ring-accent-danger/50 animate-border-breathe' : ''}`}>
      {/* 背景渐变 */}
          ...
          <div className="relative">
            <svg className="w-28 h-28 lg:w-36 lg:h-36 -rotate-90">
              ...
              <circle
                cx="50%" cy="50%" r="45%"
                fill="none"
                stroke={scoreResult.totalScore >= 80 ? '#ff4444' : scoreResult.totalScore >= 60 ? '#ffaa00' : '#00cc66'}
                strokeWidth="8"
                strokeLinecap="round"
                strokeDasharray={`${(scoreResult.totalScore / 100) * 283} 283`}
                className="transition-all duration-1000"
                style={{
                  filter: scoreResult.totalScore >= 80 ? 'drop-shadow(0 0 8px rgba(255, 68, 68, 0.6))' : 'none'
                }}
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] lg:text-xs text-text-muted">80+评分</span>
            </div>
          </div>
      ...
      <div className="p-3 rounded-lg bg-bg-component mb-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <span className="font-mono text-sm text-text-primary">{odds?.handicap?.home?.toFixed(2)}</span>
            {getTrendIcon(odds?.handicap?.homeTrend)}
          </div>
          <div className="flex items-center gap-1">
            {getTrendIcon(odds?.handicap?.awayTrend)}
            <span className="font-mono text-sm text-text-primary">{odds?.handicap?.away?.toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div className={`p-3 rounded-lg ${odds?.overUnder?.overTrend === 'down' ? 'bg-accent-success/10 border border-accent-success/30' : 'bg-bg-component'}`}>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`font-mono text-sm ${odds?.overUnder?.overTrend === 'down' ? 'text-accent-success' : 'text-accent-primary'}`}>
            {odds?.overUnder?.total}
          </span>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <span className="text-xs text-text-muted">大</span>
            <span className="font-mono text-sm text-text-primary">{odds?.overUnder?.over?.toFixed(2)}</span>
            {getTrendIcon(odds?.overUnder?.overTrend)}
          </div>
          <div className="flex items-center gap-1">
            {getTrendIcon(odds?.overUnder?.underTrend)}
            <span className="font-mono text-sm text-text-primary">{odds?.overUnder?.under?.toFixed(2)}</span>
