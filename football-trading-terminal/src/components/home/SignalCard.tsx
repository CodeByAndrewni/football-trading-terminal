...
import { MODULE_NAMES, ACTION_LABELS } from '../../types/unified-scoring';
...
function ActionBadge({ action }: ActionBadgeProps) {
  const { label, color, bgColor } = ACTION_LABELS[action];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-5 gap-2">
            ...
            <div className="p-3 rounded-lg bg-white/5">
              ...
              <div className="text-lg font-bold">{score_breakdown.timing.window_score.toFixed(1)}</div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-lg bg-white/5">
              ...
              <div className="text-xl font-bold">{reasons.stats.shots_total}</div>
              <div className="text-xs text-white/40">射正 {reasons.stats.shots_on_total}</div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
              <div className="text-xs text-white/50">xG</div>
              <div className="text-xl font-bold">{reasons.stats.xg_total.toFixed(2)}</div>
              <div className="text-xs text-white/40">欠债 {reasons.stats.xg_debt.toFixed(2)}</div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
              <div className="text-xs text-white/50">角球</div>
              <div className="text-xl font-bold">{reasons.stats.corners_total}</div>
              <div className="text-xs text-white/40">控球 {reasons.stats.possession_home}%</div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="p-3 rounded-lg bg-white/5">
              ...
              <div className="text-xl font-bold">{reasons.deltas.shots_last_15}</div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
              <div className="text-xs text-white/50">射门变化</div>
              <div className="text-xl font-bold" style={{
                color: reasons.deltas.shots_delta > 0 ? '#22c55e' : reasons.deltas.shots_delta < 0 ? '#ef4444' : undefined
              }}>
                {reasons.deltas.shots_delta > 0 ? '+' : ''}{reasons.deltas.shots_delta}
              </div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
              <div className="text-xs text-white/50">压力方向</div>
              <div className="text-sm font-bold">{reasons.deltas.pressure_direction}</div>
            </div>
            <div className="p-3 rounded-lg bg-white/5">
              <div className="text-xs text-white/50">趋势</div>
              <div className="text-sm font-bold" style={{
                color: reasons.deltas.momentum_trend === 'INCREASING' ? '#22c55e' :
                       reasons.deltas.momentum_trend === 'DECREASING' ? '#ef4444' : undefined
              }}>
                {reasons.deltas.momentum_trend}
              </div>
            </div>
          </div>
        </div>
        ...
        {/* 市场数据 */}
        {reasons.checks.has_odds && (
          <div className="mb-6">
            <h3 className="text-sm font-medium text-white/80 mb-3">市场数据</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-white/5">
                <div className="text-xs text-white/50">大球赔率</div>
                <div className="text-xl font-bold">{reasons.market.over_odds?.toFixed(2) ?? '-'}</div>
              </div>
              <div className="p-3 rounded-lg bg-white/5">
                <div className="text-xs text-white/50">隐含概率</div>
                <div className="text-xl font-bold">{reasons.market.implied_over_prob ?? '-'}%</div>
              </div>
              <div className="p-3 rounded-lg bg-white/5">
                <div className="text-xs text-white/50">盘口变化</div>
                <div className="text-sm font-bold" style={{
                  color: reasons.market.line_movement === 'DOWN' ? '#22c55e' :
                         reasons.market.line_movement === 'UP' ? '#ef4444' : undefined
                }}>
                  {reasons.market.line_movement ?? '-'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <span className={`px-2 py-1 rounded text-xs ${reasons.checks.has_stats ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
              统计 {reasons.checks.has_stats ? '✓' : '✗'}
            </span>
            <span className={`px-2 py-1 rounded text-xs ${reasons.checks.has_events ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
              事件 {reasons.checks.has_events ? '✓' : '✗'}
            </span>
            <span className={`px-2 py-1 rounded text-xs ${reasons.checks.has_odds ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
              赔率 {reasons.checks.has_odds ? '✓' : '✗'}
            </span>
            <span className={`px-2 py-1 rounded text-xs ${reasons.checks.stats_fresh ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
              新鲜 {reasons.checks.stats_fresh ? '✓' : '✗'}
            </span>
            {reasons.checks.data_anomaly && (
              <span className="px-2 py-1 rounded text-xs bg-red-500/20 text-red-400">
                异常: {reasons.checks.anomaly_reason}
              </span>
            )}
          </div>
        </div>
        ...
        {reasons.tags.length > 0 && (
          <div className="mb-6">
            ...
            <TagList tags={reasons.tags} maxShow={10} />
          </div>
        )}
    ...
    <div className="p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/8 transition-colors">
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-center gap-2 text-xs">
          ...
          <ScoreBar score={signal.score_breakdown.edge.total} maxScore={30} />
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-white/40 w-12">Timing</span>
          <ScoreBar score={signal.score_breakdown.timing.window_score} maxScore={20} />
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-white/40 w-12">Market</span>
          <ScoreBar score={signal.score_breakdown.market.total} maxScore={20} />
        </div>
      </div>

      {/* 标签 */}
      {signal.reasons.tags.length > 0 && (
        <div className="mb-4">
